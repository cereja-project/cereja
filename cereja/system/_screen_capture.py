"""Windows desktop capture with owned BGRA frames and no third-party runtime.
...
class _Win32:
    """Private, typed native bindings, loaded only when capture is requested."""

    def __init__(self):
        if sys.platform != "win32":
            raise OSError("ScreenCapture requires Windows 10 or newer")
        user = ctypes.WinDLL("user32", use_last_error=True)
        gdi = ctypes.WinDLL("gdi32", use_last_error=True)
        self.monitor_callback = ctypes.WINFUNCTYPE(
        ...
        bind(user, "SetThreadDpiAwarenessContext", wintypes.HANDLE, wintypes.HANDLE)
        bind(user, "EnumDisplayMonitors", wintypes.BOOL, wintypes.HDC,
             ctypes.POINTER(wintypes.RECT), self.monitor_callback, wintypes.LPARAM)
        bind(user, "GetMonitorInfoW", wintypes.BOOL, wintypes.HANDLE,
        ...
        bind(user, "DestroyIcon", wintypes.BOOL, wintypes.HANDLE)
        bind(user, "DrawIconEx", wintypes.BOOL, wintypes.HDC, ctypes.c_int,
             ctypes.c_int, wintypes.HANDLE, ctypes.c_int, ctypes.c_int,
             wintypes.UINT, wintypes.HBRUSH, wintypes.UINT)
        bind(gdi, "CreateCompatibleDC", wintypes.HDC, wintypes.HDC)
        bind(gdi, "CreateDIBSection", wintypes.HBITMAP, wintypes.HDC,
        ...
        bind(gdi, "BitBlt", wintypes.BOOL, wintypes.HDC, ctypes.c_int,
             ctypes.c_int, ctypes.c_int, ctypes.c_int, wintypes.HDC,
