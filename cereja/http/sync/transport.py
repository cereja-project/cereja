...
from ..errors import BodyLimitExceeded, ConnectError, ConnectTimeout, ProtocolError, ReadTimeout, TLSFailure
...
class SyncTransport:
    ...
    def send(self, request, timeout, *, stream=False):
        ...
        if length is not None and self.max_body_bytes is not None and int(length) > self.max_body_bytes:
            raw.close()
            abort()
        ...
        try:
            ...
            if length is not None and total != int(length):
                raise ProtocolError(f"Premature EOF: expected {length} bytes, got {total}")
        except socket.timeout as exc:
            raw.close()
            abort()
            raise ReadTimeout(f"Timed out reading from {request.url}") from exc
